export function calculateFare (segments: any[]) {
	...
	for (const segment of segments) {
		...
		if (isOvernight(segment.date) && !isSunday(segment.date)) fare += segment.distance * 3.90;
		if (isOvernight(segment.date) && isSunday(segment.date)) fare += segment.distance * 5;
		if (!isOvernight(segment.date) && isSunday(segment.date)) fare += segment.distance * 2.9;
		if (!isOvernight(segment.date) && !isSunday(segment.date)) fare += segment.distance * 2.10;
	}
