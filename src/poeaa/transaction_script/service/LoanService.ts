...
import {
  saveInstallmentsUsingSAC,
  saveInstallmentsUsingPrice,
...
export default class LoanService {
  async applyForLoan(loan: Loan) {
    ...
    if (loan.type === 'mortgage') {
      if (loan.installments > 420)
        throw new Error(
          'The maximum number of installments for mortgage loan is 420',
        );
      if (loan.income * 0.25 < loan.amount / loan.installments)
        throw new Error(
    ...
    if (loan.type === 'car') {
      if (loan.installments > 60)
        throw new Error(
          'The maximum number of installments for car loan is 60',
        );
      if (loan.income * 0.3 < loan.amount / loan.installments)
        throw new Error(
